...
export class OrderService {
  ...
  async findAll({ limit, offset }): Promise<PaginationResult<Order>> {
    ...
    const results = await this.orderModel.find({}, {}, { limit, skip: offset }).populate(['user', 'courier', 'store', 'products.product']).exec();
  ...
  async findAllByUserId({ limit, offset, id }) {
    ...
    const results = await this.orderModel.find({user: id}, {}, { limit, skip: offset }).populate(['user', 'courier', 'store', 'products.product']).exec();
